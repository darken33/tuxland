...
/**
 * XBill - Ennemi de Tux ce deplaçant de droite a gauche
 * @author Philippe Bousquet (Darken33@free.fr)
 * @version 0.1.0
 */
public class XBill extends Enemy 
{
     ...
     */
    public XBill(String name, int x, int y, int orientation)
    {
	  super(name,x,y,orientation);
    } 

    /**
     * autoMove : Déplacer automatiquement XBill
     */
    public void autoMove()
    {
	  Point p = getMapPosition();
	  int direction = getXOrientation();
	  if ((isStopMonster(p.x+2*direction,p.y)) || (isSolid(p.x+2*direction,p.y)) || (isEmpty(p.x+direction,p.y+1))) direction=-direction;
